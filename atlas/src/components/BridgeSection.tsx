                    ...
                    <>
                      ...
                      {bridge.parts.map((part) => (
                        <button
                          ...
                          className={`absolute transition-all duration-500 ease-out cursor-pointer group/part flex items-center justify-center`}
                          style={{
                            top: part.position.top,
                            left: part.position.left,
                            width: hoveredPart === part.id ? '120px' : '60px',
                            height: hoveredPart === part.id ? '120px' : '60px',
                            transform: hoveredPart === part.id ? 'translate(-50%, -50%) scale(1.4)' : 'translate(-50%, -50%)',
                            borderRadius: '50%',
                            ...
                            zIndex: hoveredPart === part.id ? 40 : 20
                          }}
                        >
                          <span className="absolute inset-0 flex items-center justify-center text-white text-sm font-sans-cn opacity-0 group-hover/part:opacity-100 transition-all duration-300 pointer-events-none">
                            {part.name}
                          </span>
                        </button>
                      ))}
...
const MetaItem = ({ label, value, hasInfo, onShowInfo }: { label: string; value: string; hasInfo?: boolean; onShowInfo?: () => void }) => (
  <div>
    ...
    <p 
      className={`text-lg font-medium text-ink font-sans-cn mt-0.5 ${hasInfo ? 'cursor-pointer hover:text-cinnabar transition-colors hover:scale-110 transition-transform' : ''}`}
  ...
  return (
    <div>
      <div className="flex justify-between items-baseline mb-2">
        <span className="text-base text-muted-foreground font-sans-cn">{label}</span>
        <span className="text-xl md:text-3xl font-serif-cn font-bold text-ink tabular-nums">{value}</span>
      </div>
